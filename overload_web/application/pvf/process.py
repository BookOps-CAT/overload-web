...
class ProcessAcquisitionsRecords:
    ...
    def execute(
        ...
        marc_parsing_rules: dict[str, Any],
        ...
        Args:
            ...
            marc_parsing_rules:
                a dictionary containing cataloging rules for MARC parsing.
            marc_update_rules:
        ...
        parser = marc.BibParser(**marc_parsing_rules)
        ...
        for file_name, data in batches.items():
            ...
            records = parser.parse_marc_data(
                handler=marc_parser, data=data, vendor=vendor
            )
            ...
            for bib in records:
                ...
                update.BibRecordUpdater.update_record(
                    bib, handler=marc_updater, updates=update_fields
                )
...
class ProcessCatalogingRecords:
    ...
    def execute(
        ...
        parser = marc.BibParser(**marc_parsing_rules)
        data = parser.combine_marc_files(data=content, handler=marc_parser)
        records = parser.parse_marc_data(handler=marc_parser, data=data)
        ...
        deduplicated = update.BibRecordUpdater.deduplicate(
            records=records, handler=marc_updater
        )
