...
class TestUpdater:
    ...
    def test_update_cat_bpl(self, full_bib, test_constants):
        original_bib = Bib(
    ...
    def test_update_cat_nypl(self, full_bib, test_constants):
        original_bib = Bib(
            copy.deepcopy(full_bib.binary_data), library=full_bib.library
        )
        full_bib.bib_id = "12345"
        service = update.FullLevelBibUpdater(
            handler=marc_updater.BookopsMarcUpdateHandler(
                library="nypl", record_type="cat", rules=test_constants
            )
        )
        updated_bibs = service.update([full_bib])
        updated_bib = Bib(updated_bibs[0].binary_data, library=updated_bibs[0].library)
    ...
    def test_update_cat_vendor_updates(self, full_bib, library, test_constants):
