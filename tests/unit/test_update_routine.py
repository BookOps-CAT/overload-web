...
class TestUpdater:
    ...
    def test_update_template_data_acq(self, acq_bib, marc_engine):
        ...
        acq_bib.apply_order_template(
            {"name": "Foo", "order_code_1": "b", "format": "a"}
        )
        marc_record = marc_engine.create_bib_from_domain(record=acq_bib)
        updates = rules.VendorRules.fields_to_update(
            record=acq_bib, context=marc_engine._config, format="a"
        )
        marc_engine.update_fields(field_updates=updates, bib=marc_record)
        marc_record.leader = rules.FieldRules.update_leader(marc_record.leader)
        acq_bib.binary_data = marc_record.as_marc()
    ...
    def test_update_template_data_sel(self, sel_bib, marc_engine):
        ...
        sel_bib.apply_order_template(
            {"name": "Foo", "order_code_1": "b", "format": "a"}
        )
        marc_record = marc_engine.create_bib_from_domain(record=sel_bib)
        updates = rules.VendorRules.fields_to_update(
            record=sel_bib, context=marc_engine._config, format="a"
        )
        marc_engine.update_fields(field_updates=updates, bib=marc_record)
        marc_record.leader = rules.FieldRules.update_leader(marc_record.leader)
        sel_bib.binary_data = marc_record.as_marc()
        assert [i.order_code_1 for i in original_orders] == ["j"]
